...
class BaseParserResumeHTML(object):
    ...
    def get_gender(self, html):
        value = self._get_target('gender', html)
        try:
            gender = value[0].upper()
        except IndexError:
            gender = None
        return gender
...
def parser_resume(cls=None, html=None):
    ...
    length_of_work_html = cls.get_container_length_of_work(html)
    length_of_work = cls.get_length_of_work(length_of_work_html)
    resume_data.setdefault('length_of_work', length_of_work)
...
class HhParserSearch(BaseParserSearchHTML):
    ...
    container_body = Expression(tag='div',
                                attribute='data-qa',
...
class SearchBase(object):
    ...
    def _generate_search_url(self, search_str):
    ...
    def _next_step(self, search_url=None, namber_page=0):
        if not search_url:
    ...
    def search(self, search_str=None, session=None, resume_list=[], 
        ...
        if not (reload_error_flag or update_flad):

            if not search_str:
                raise SearchError("Search_str is None")

            search_url = self._generate_search_url(search_str)
            ...
            while True:
                search_speak = self._next_step(search_url, namber_page)
        ...
        if len(resume_list) > 0:
            ...
            for resume in resume_list:
                ...
                resume_data.setdefault('search_text', search_str)
                resume_data_list.append(resume_data)

        return resume_data_list, resume_error_list

    def resume_html(self, resume_url, session=None):
        if not session:
            session = requests.Session()
        ...
        resume_request = session.prepare_request(resume_reque_obj)
        try:
            resume_connect = session.send(resume_request)
        except HTTPError:
            resume_connect = None
        except ConnectionError:
            resume_connect = None
        ...
        if resume_connect:
            resume_html = resume_connect.text
            resume_connect.close()
        else:
            resume_html = None
...
class SearchRabota(SearchBase):
    pass


class SearchFarpost(SearchBase):
    pass


class SearchRabotavgorode(SearchBase):
    pass
'''
...
class SearchZarplata(SearchBase):
    pass
...
class AuthBase(object):
    ...
    def auth(self, login=None, password=None):
        if not login or not password:
            raise AuthError("Login or Password not defined")

        auth_url = getattr(self, 'auth_url', None)

        if not auth_url:
            raise AuthError("Auth URL is not defined")

        credentials = getattr(self, 'credentials', None)

        if not credentials:
            raise AuthError("Credentials is empty")

        headers = getattr(self, 'headers', None)

        if not headers:
            raise AuthError("Headers is empty")

        session = requests.Session()
        session.headers = headers

        login_get = requests.Request('GET', auth_url)
        session_request_get = session.prepare_request(login_get)
        responce_get = session.send(session_request_get)
        responce_get.close()

        if responce_get.status_code != 200:
            raise AuthError("Auth failed, responce status = %i" %(
                            responce_get.status_code))

        login_key = getattr(self, 'login_key', None)
        password_key = getattr(self, 'password_key', None)

        if not login_key or not password_key:
            raise AuthError("'login_key' or 'password_key' is empty")

        credentials[login_key] = login
        credentials[password_key] = password

        login_post = requests.Request('POST', auth_url, data=credentials)
        session_request_post = session.prepare_request(login_post)
        responce_post = session.send(session_request_post)
        responce_post.close()

        if responce_post.status_code != 200:
            #raise AuthError("Auth is failed, responce statue = %i" %(
            #                responce_post.status_code))
...
class AuthHh(AuthBase):
    headers = {
       ...
       'DNT':'1', 
       'Referer':'https://hh.ru/login',
       'Upgrade-Insecure-Requests':'1'
    }
    auth_url = 'https://hh.ru/account/login?backurl=%2F'
    test_url = 'https://hh.ru/employer/vacancies'
    credentials = {
        'remember': 'yes',
        'backUrl': 'https://hh.ru/',
...
class AuthSj(AuthBase):
    pass


class AuthAvito(AuthBase):
    pass


class AuthRabota(AuthBase):
    pass


class AuthFarpost(AuthBase):
    pass


class AuthRabotavgorode(AuthBase):
    pass


class AuthZarplata(AuthBase):
    pass
...
'''
    ...
    auth_hh = AuthHh()
    session = auth_hh.auth('alipskiy@sentinelcredit.ru', '123456')
    print(session.cookie.get_dick)
